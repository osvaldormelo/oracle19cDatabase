...
To run your Oracle Database 21c, or 18c Express Edition container image use the `podman run` command as follows:
...
To run your Oracle Database Express Edition container image use the `podman run` command as follows:
...
The password for those accounts can be changed via the `podman exec` command. **Note**, the container has to be running:
    ...
    podman exec <container name> /u01/app/oracle/setPassword.sh <your password>
...
```bash
podman run --name <container-name> -e CLONE_DB=true \
...
You may use the same container image you used to start the database, to run `sqlplus` to connect to it, for example:
    ...
    podman run --rm -ti oracle/database:19.3.0-ee sqlplus pdbadmin/<yourpassword>@//<db-container-ip>:1521/ORCLPDB1

Another option is to use `podman exec` and run `sqlplus` from within the same container already running the database:

    podman exec -ti <container name> sqlplus pdbadmin@ORCLPDB1
...
The example below mounts the local directory myScripts to `/opt/oracle/myScripts` which is then searched for custom startup scripts:
    ...
    podman run --name oracle-ee -p 1521:1521 -v /home/oracle/myScripts:/opt/oracle/scripts/startup -v /home/oracle/oradata:/opt/oracle/oradata oracle/database:19.3.0-ee
